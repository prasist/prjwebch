...
use App\Models\celulas_nivel5;
use URL;
use Auth;
use Input;
use Gate;

class Estruturas5Controller extends Controller
{

    public function __construct()
    {

        $this->rota = "estruturas5"; //Define nome da rota que será usada na classe
    ...
    public function index()
    {
        ...
        /*Busca NIVEL5*/
        $dados = \DB::select('select * from view_celulas_nivel5 v5 where  v5.empresas_id = ? and v5.empresas_clientes_cloud_id = ? ', [$this->dados_login->empresas_id, $this->dados_login->empresas_clientes_cloud_id]);
  ...
  public function salvar($request, $id, $tipo_operacao)
  {
        ...
        {
             $dados = new celulas_nivel5();
        }
        else //update
        {
             $dados = celulas_nivel5::findOrfail($id);
        }
    ...
    public function create()
    {
        ...
        return view($this->rota . '.registrar', ['nivel1'=>$view1, 'nivel2'=>$view2, 'nivel3'=>$view3, 'nivel4'=>$view4]);
...
*/
    ...
    private function exibir ($request, $id, $preview)
    {
       ...
       /*Busca NIVEL2*/
        ...
        /*Busca NIVEL4*/
        $dados = \DB::select('select * from view_celulas_nivel5 v5 where v5.id = ? and v5.empresas_id = ? and v5.empresas_clientes_cloud_id = ? ', [$id, $this->dados_login->empresas_id, $this->dados_login->empresas_clientes_cloud_id]);

        return view($this->rota . '.edit', ['dados' =>$dados, 'preview' => $preview, 'nivel1' =>$view1, 'nivel2' =>$view2, 'nivel3' =>$view3, 'nivel4' =>$view4]);
