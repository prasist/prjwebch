...
class ControleAtividadesController extends Controller
{
    ...
    public function index()
    {
        ...
        return view($this->rota . '.atualizacao', ['preview'=>'false', 'tipo_operacao'=>'incluir', 'celulas'=>$celulas]);
  ...
  public function salvar($request, $id, $tipo_operacao)
  {
        ...
        else //update
        {
         ...
         $dados->controle_atividades_nivel1_id  = ($input['nivel1']=="" ? null : $input['nivel1']);
         $dados->controle_atividades_nivel2_id  = ($input['nivel2']=="" ? null : $input['nivel2']);
         $dados->controle_atividades_nivel3_id  = ($input['nivel3']=="" ? null : $input['nivel3']);
         $dados->controle_atividades_nivel4_id  = ($input['nivel4']=="" ? null : $input['nivel4']);
         $dados->controle_atividades_nivel5_id  = ($input['nivel5']=="" ? null : $input['nivel5']);
         ...
         $dados->controle_atividades_pai_id = ($input['controle_atividades_pai_id']=="" ? null : $input['controle_atividades_pai_id']);
    ...
    public function create()
    {
        ...
        /*Busca NIVEL5*/
        $view5 = \DB::select('select * from view_controle_atividades_nivel5 v5 where v5.empresas_id = ? and v5.empresas_clientes_cloud_id = ? ', [$this->dados_login->empresas_id, $this->dados_login->empresas_clientes_cloud_id]);

        //return view($this->rota . '.registrar', ['nivel5'=>$view5, 'publicos'=>$publicos, 'faixas'=>$faixas]);
        return view($this->rota . '.atualizacao', ['preview'=>'false', 'nivel5'=>$view5, 'publicos'=>$publicos, 'faixas'=>$faixas, 'tipo_operacao'=>'incluir', 'dados'=>$vazio, 'controle_atividades'=>$controle_atividades, 'vinculos'=>$vazio, 'total_vinculos'=>'0']);
...
*/
    public function store(\Illuminate\Http\Request  $request)
    {
            ...
            {
                return redirect('controle_atividadespessoas/registrar/' . $id_gerado);
            }
            else
            {
    ...
    private function exibir ($request, $id, $preview)
    {
        ...
        /*Busca NIVEL4*/
        $dados = \DB::select("select to_char(to_date(data_inicio, 'yyyy-MM-dd'), 'DD/MM/YYYY') AS data_inicio_format, * from view_controle_atividades  where id = ? and empresas_id = ? and empresas_clientes_cloud_id = ? ", [$id, $this->dados_login->empresas_id, $this->dados_login->empresas_clientes_cloud_id]);

        //Busca controle_atividades filhas
        $vinculos = \DB::select('select * from view_controle_atividades_simples  where controle_atividades_pai_id = ?  and empresas_id = ? and empresas_clientes_cloud_id = ? ', [$id, $this->dados_login->empresas_id, $this->dados_login->empresas_clientes_cloud_id]);
        ...
        {
            $temp = \DB::select('select count(*) as tot from view_controle_atividades  where controle_atividades_pai_id = ?  and empresas_id = ? and empresas_clientes_cloud_id = ? ', [$id, $this->dados_login->empresas_id, $this->dados_login->empresas_clientes_cloud_id]);
        ...
        return view($this->rota . '.atualizacao', ['dados' =>$dados, 'preview' => $preview,  'nivel5' =>$view5, 'publicos'=>$publicos, 'faixas'=>$faixas, 'tipo_operacao'=>'editar', 'vinculos'=>$vinculos, 'controle_atividades'=>$controle_atividades, 'total_vinculos'=>$total_vinculos]);
    ...
    public function update(\Illuminate\Http\Request  $request, $id)
    {
           ...
           if ($request["quero_incluir_participante"]=="sim")
            {
                return redirect('controle_atividadespessoas/' . $id . '/edit');
            }
            else
            {
                return redirect($this->rota);
